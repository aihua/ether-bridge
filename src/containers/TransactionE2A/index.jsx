            ...
            <div className='e2a'>
                <div className="transactionMeta" onClick={this.toggleDetails}>
                    <div className="transactionMetaInfo">
                        <label>-{this.parseValue(this.props.value)} eth -> +{this.parseValue(this.props.value)} ebc </label>
                    ...
                    <div className="transctionMetaStatus">
                        ...
                        <div className="transctionMetaSingleStatus">
                            ...
                            {(this.getStatusNum(this.props.status) === 2 || this.getStatusNum(this.props.status) === 3) && <img src={"/Rectangle6.png"} alt="disabled"/>}
                        ...
                        <div className="transctionMetaSingleStatus">
                            ...
                            {this.getStatusNum(this.props.status) === 2 && <img src={"/Rectangle6_disable.png"} alt="disabled"/>}
                            ...
                            {this.getStatusNum(this.props.status) === 4 && <img src={"/Rectangle6_disable.png"} alt="disabled"/>}
                        </div>
                    </div>
                </div>
                ...
                <div className="transactionDetail">
                    <div className="transactionDetailItems">
                        ...
                        <div className="transactionDetailSingleItem">
                            ...
                            {this.getStatusNum(this.props.status) === 1 && <label style={{float:'right'}}>{this.getBlockNumber() - this.props.eth_block_num}/30</label>}
                            {(this.getStatusNum(this.props.status) > 1 && this.getStatusNum(this.props.status) < 5) && <label style={{float:'right'}}>30/30</label>}
                        ...
                        <div className="transactionDetailSingleItem">
                            ...
                            {(this.getStatusNum(this.props.status) > 1 && this.getStatusNum(this.props.status) < 4) && <label style={{float:'right'}}>Completed</label>}
                            {/* 5 状态表示失败，可能原因很多，显示失败状态在这里 */}
                            {this.getStatusNum(this.props.status) === 4 && <label style={{float:'right'}}>Failed</label>}
                        </div>
                        <div className="transactionDetailSingleItem">
                            <label>交易哈希：</label>
                            {/* 初始状态（即已经获取到了eth hash和blockNum），还未发起，此时状态为NA */}
                            ...
                            {(this.getStatusNum(this.props.status) > 1 && this.getStatusNum(this.props.status) < 4) && <label style={{float:'right'}}>{this.props.ac_tx_hash}</label>}
                            {/* 4 状态表示失败，可能有hash也可能没有，显示或不显示都在后mark为失败 */}
                            {this.getStatusNum(this.props.status) === 4 && <label style={{float:'right'}}>{this.props.ac_tx_hash} (Failed)</label>}
                        </div>
                    </div>
                </div>
                }
